...
<style>
    ...
    .metric-card { background: #1e293b; border-radius: 10px; padding: 16px; border: 1px solid #2d3748; text-align: center; }
...
def load_model():
    ...
    try:
        if os.path.exists("saved_model.keras"):
            return tf.keras.models.load_model("saved_model.keras", custom_objects=custom_objects)
...
FEATURE_NAMES = [
    ...
    "roll_mean_10", "roll_std_10", "spike_flag", "cpu_diff1", "cpu_diff3"
]
N_FEATURES = 15
...
def build_feature_window(cpu_array):
    ...
    for i, c in enumerate(cpu_array):
        ...
        features.append([c, hour_sin, hour_cos, dow_sin, dow_cos,
                         lag1, lag5, lag10, lag2, lag3,
                         roll_mean, roll_std, spike_flag, cpu_diff1, cpu_diff3])
...
def multistep_predict(model, history, scaler_live, cpu_scaler_live, n_steps=5, window_size=60):
    if model is None or len(history) < window_size:
        return [], []
    means, stds = [], []
...
with st.sidebar:
    ...
    slack_webhook = st.text_input("🔔 Slack Webhook URL", value=default_webhook,
                                   placeholder="https://hooks.slack.com/services/...", type="password")
    if slack_webhook:
        if st.button("🧪 Test Slack"):
            try:
                r = requests.post(slack_webhook, json={"text": "✅ CrashGuard AI — Slack connected!"}, timeout=5)
...
<div style='background:#111827; padding:12px 20px; border-radius:10px;
    ...
    <div style='display:flex; align-items:center; gap:12px;'>
        ...
        <div>
            ...
            <span style='font-size:12px; color:#64748b; margin-left:10px;'>CPU Observability Platform — Predictive Alerting</span>
        </div>
    </div>
    <div style='font-size:12px; color:#64748b;'>{now.strftime("%H:%M:%S")} | Node: node-1 | Uptime: Active</div>
</div>
""", unsafe_allow_html=True)

c1, c2, c3, c4, c5, c6 = st.columns(6)
with c1:
    st.markdown(f"""<div class='metric-card'><div class='metric-label'>System Health</div>
        <div class='metric-value' style='color:{status_color}; font-size:20px;'>{status_emoji} {status}</div></div>""", unsafe_allow_html=True)
with c2:
    cc = "#ef4444" if current_cpu > 0.8 else "#f59e0b" if current_cpu > 0.6 else "#22c55e"
    st.markdown(f"""<div class='metric-card'><div class='metric-label'>Current CPU</div>
        <div class='metric-value' style='color:{cc};'>{current_cpu:.1%}</div></div>""", unsafe_allow_html=True)
with c3:
    pc = "#ef4444" if predicted_cpu > 0.8 else "#f59e0b" if predicted_cpu > 0.6 else "#22c55e"
    st.markdown(f"""<div class='metric-card'><div class='metric-label'>Predicted CPU</div>
        <div class='metric-value' style='color:{pc};'>{predicted_cpu:.1%}</div></div>""", unsafe_allow_html=True)
with c4:
    spike_text = f"{time_to_spike // 60}min" if time_to_spike else "No spike"
    spike_color = "#ef4444" if time_to_spike else "#22c55e"
    st.markdown(f"""<div class='metric-card'><div class='metric-label'>Time to Spike</div>
        <div class='metric-value' style='color:{spike_color}; font-size:22px;'>{spike_text}</div></div>""", unsafe_allow_html=True)
with c5:
    st.markdown(f"""<div class='metric-card'><div class='metric-label'>Confidence</div>
        <div class='metric-value' style='color:{conf_color};'>{conf_val:.2f}</div></div>""", unsafe_allow_html=True)
with c6:
    st.markdown(f"""<div class='metric-card'><div class='metric-label'>Alerts Fired</div>
        <div class='metric-value' style='color:#f59e0b;'>{st.session_state.alert_count}</div></div>""", unsafe_allow_html=True)
...
with left_col:
    ...
    fig = go.Figure()
    if len(cpu_vals) > 1:
        fig.add_trace(go.Scatter(x=list(range(len(cpu_vals))), y=cpu_vals, name="Live CPU",
                                  line=dict(color="#3b82f6", width=2), fill="tozeroy",
                                  fillcolor="rgba(59,130,246,0.08)"))
    fig.add_hline(y=spike_thresh, line_dash="dash", line_color="#ef4444", opacity=0.5,
                   annotation_text=f"Spike Threshold ({spike_thresh:.0%})", annotation_font_color="#ef4444")
    if multistep_means:
        future_x = [len(cpu_vals) + i for i in range(len(multistep_means))]
        fig.add_trace(go.Scatter(x=future_x, y=multistep_means, name="Predicted",
        ...
        fig.add_trace(go.Scatter(x=future_x + future_x[::-1], y=upper + lower[::-1],
                                  fill="toself", fillcolor="rgba(245,158,11,0.15)",
                                  line=dict(color="rgba(0,0,0,0)"), name="95% CI"))
        for i, p in enumerate(multistep_means):
            if p > spike_thresh:
                fig.add_trace(go.Scatter(x=[future_x[i]], y=[p], mode="markers",
                                          marker=dict(color="#ef4444", size=14),
                                          name="Spike Predicted", showlegend=(i == 0)))
    fig.update_layout(title=dict(text="Real-Time CPU Forecast", font=dict(color="#e2e8f0", size=15)),
                       paper_bgcolor="#111827", plot_bgcolor="#0a0e1a", font=dict(color="#94a3b8"),
                       xaxis=dict(gridcolor="#1e293b", title="Time Steps"),
                       yaxis=dict(gridcolor="#1e293b", title="CPU Usage", range=[0, 1.05], tickformat=".0%"),
    ...
    with core_col:
        st.markdown("<div class='section-header'>⚡ Per-Core CPU Usage</div>", unsafe_allow_html=True)
        core_pcts = psutil.cpu_percent(interval=0.5, percpu=True)
        if len(core_pcts) > 0:
            colors_c = ["#ef4444" if c > 80 else "#f59e0b" if c > 60 else "#22c55e" for c in core_pcts]
            fig_cores = go.Figure()
            fig_cores.add_trace(go.Bar(x=[f"C{i}" for i in range(len(core_pcts))], y=core_pcts,
                                        marker_color=colors_c, text=[f"{c:.0f}%" for c in core_pcts],
    ...
    with model_col:
        ...
        st.markdown(f"""
        <div style='background:#111827; border:1px solid #1e293b; border-radius:8px; padding:14px; font-size:13px;'>
            <div style='margin-bottom:8px;'><span style='color:#64748b;'>Model:</span><span style='color:#e2e8f0; float:right;'>LSTM + XGBoost</span></div>
            <div style='margin-bottom:8px;'><span style='color:#64748b;'>Status:</span><span style='color:#22c55e; float:right;'>{'✅ Loaded' if model else '❌ Not loaded'}</span></div>
            <div style='margin-bottom:8px;'><span style='color:#64748b;'>Scaler:</span><span style='color:#22c55e; float:right;'>{'✅ Loaded' if scaler_live else '❌ Missing'}</span></div>
            <div style='margin-bottom:8px;'><span style='color:#64748b;'>Confidence:</span><span style='color:{conf_color}; float:right;'>{conf_val:.3f}</span></div>
            <div style='margin-bottom:8px;'><span style='color:#64748b;'>Explainability:</span><span style='color:#22c55e; float:right;'>{'✅ IG Active' if st.session_state.ig_values is not None else '⏳ Computing'}</span></div>
            <div style='margin-bottom:8px;'><span style='color:#64748b;'>Window:</span><span style='color:#e2e8f0; float:right;'>{window_size} steps</span></div>
            <div><span style='color:#64748b;'>MC Samples:</span><span style='color:#e2e8f0; float:right;'>{n_mc_samples}</span></div>
...
with right_col:
    ...
    if st.session_state.alerts:
        for a in st.session_state.alerts[:3]:
            ...
            <div class='alert-{sev.lower()}'>
                ...
                <div style='color:#e2e8f0;font-size:12px;margin:4px 0;'>Predicted: {a['predicted']:.1%} | Conf: {a['confidence']:.2f}</div>
                <div style='font-size:11px;color:#64748b;'>{a['timestamp']}</div>
                <hr style='border-color:#2d3748;margin:6px 0;'>
                <div style='font-size:11px;color:#64748b;margin-bottom:2px;'>CAUSES</div>{causes_html}
                <div style='font-size:11px;color:#64748b;margin-top:6px;margin-bottom:2px;'>ACTIONS</div>{actions_html}
            </div>""", unsafe_allow_html=True)
    else:
        st.markdown("""<div style='background:#111827;border:1px solid #1e293b;border-radius:8px;padding:20px;text-align:center;color:#64748b;font-size:13px;'>✅ No alerts<br>System operating normally</div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("<div class='section-header'>🔍 Why This Prediction? (Integrated Gradients)</div>", unsafe_allow_html=True)
    ...
    if st.session_state.ig_values is not None:
        ...
        colors_ig = ["#ef4444" if v > 0.2 else "#f59e0b" if v > 0.1 else "#3b82f6" for v in ig_vals[order]]
        fig_ig = go.Figure()
        fig_ig.add_trace(go.Bar(x=ig_vals[order], y=[ig_names[i] for i in order], orientation="h",
                                 marker_color=colors_ig, text=[f"{v:.1%}" for v in ig_vals[order]],
                                 textposition="auto", textfont=dict(size=10)))
        fig_ig.update_layout(paper_bgcolor="#111827", plot_bgcolor="#0a0e1a", font=dict(color="#94a3b8", size=10),
                              xaxis=dict(gridcolor="#1e293b", title="Attribution (IG)"),
                              yaxis=dict(gridcolor="#1e293b"), height=380,
                              margin=dict(l=90, r=10, t=30, b=30), showlegend=False,
                              title=dict(text="Feature Attribution — Integrated Gradients", font=dict(color="#94a3b8", size=11)))
        st.plotly_chart(fig_ig, use_container_width=True)
        st.markdown("<div style='font-size:10px;color:#475569;text-align:center;'>Integrated Gradients (Sundararajan et al., 2017)</div>", unsafe_allow_html=True)
    else:
        st.markdown("""<div style='background:#111827;border:1px solid #1e293b;border-radius:8px;padding:15px;text-align:center;color:#64748b;font-size:12px;'>⏳ Computing Integrated Gradients...<br>Will appear after first prediction</div>""", unsafe_allow_html=True)

    if st.session_state.last_alert_time:
        elapsed = (datetime.now() - st.session_state.last_alert_time).seconds
        remaining = max(0, ALERT_COOLDOWN - elapsed)
        if remaining > 0:
            st.markdown(f"""<div style='background:#111827;border:1px solid #1e293b;border-radius:6px;padding:8px;font-size:11px;color:#64748b;text-align:center;'>⏱️ Alert cooldown: {remaining}s</div>""", unsafe_allow_html=True)
